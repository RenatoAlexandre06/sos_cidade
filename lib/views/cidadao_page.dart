...
class _CidadaoPageState extends State<CidadaoPage> {
  ...
  Future<void> _capturarImagem(ImageSource source) async {
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Erro ao aceder à câmara/galeria: $e'),
  ...
  Future<Position?> _determinarPosicaoAtual() async {
    ...
    if (permissao == LocationPermission.denied) {
      ...
      if (permissao == LocationPermission.denied) {
        return Future.error(
          'A permissão para aceder à localização foi recusada.',
        );
      }
    }
    ...
    if (permissao == LocationPermission.deniedForever) {
      return Future.error(
        'As permissões de localização estão permanentemente recusadas.',
      );
    }
      ...
      if (mounted) {
        ...
        provider.addChamado(chamado).then((_) {
          NotificationService.exibirNotificacao(
            ...
            corpo:
                'A sua solicitação sobre "${chamado.titulo}" foi encaminhada com sucesso.',
          );
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text('Chamado enviado para a prefeitura!'),
                ...
                children: [
                  ...
                  Expanded(
                    child: lista.isEmpty
                        ? const Center(
                            child: Text(
                              'Nenhuma atualização registada ainda.',
        ...
        children: [
          Container(
            ...
            child: const Column(
              children: [
                ...
                SizedBox(height: 4),
                Text(
                  'Veja abaixo o andamento das solicitações da sua comunidade',
          ...
          if (chamados.isEmpty)
            const Padding(
              ...
              child: Center(
                child: Text('Nenhuma solicitação registada ainda.'),
              ),
            )
          else
            ListView.builder(
          ...
          children: [
            ...
            TextFormField(
              ...
              decoration: const InputDecoration(
                labelText: 'O que aconteceu? (Ex: Poste sem luz)',
            ...
            TextFormField(
              ...
              decoration: const InputDecoration(
                labelText: 'Explique em detalhes o problema',
                border: OutlineInputBorder(),
              ),
              maxLines: 4,
              validator: (value) => value == null || value.trim().isEmpty
